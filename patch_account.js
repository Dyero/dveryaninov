      ...
      var itemsHtml = (ord.items || []).map(function(it) {
        ...
        if (it.accessories && it.accessories.length) {
          ...
          it.accessories.forEach(function(acc) {
            ...
            accHtml += '<div style="font-size:13px; color:#555; margin-top:4px;"><span style="color:#999">Цена:</span> ' + acc.price + ' ₽ × ' + acc.qty + '</div>';
            accHtml += '</div></div>';
          });
          accHtml += '</div>';
        }
...
js = js.replace(/function renderOrders\(\) \{[\s\S]*?\}\n\n  function initLogout/m, newRenderCode.trim() + '\n\n  function initLogout');
fs.writeFileSync(jsPath, js);
console.log("account.js patched");
